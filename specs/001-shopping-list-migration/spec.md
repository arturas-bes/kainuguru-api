...
2. **Given** multiple alternatives exist, **When** displaying suggestions, **Then** they are ranked by score (brand match: 3.0, same store: 2.0, size match: 1.0, price proximity: 1.0) with confidence scores (0.0-1.0 scale) provided
...
- **FR-004**: System MUST NOT suggest products from more than 2 stores in a single wizard session (see US3 for handling when >2 stores needed)
- **FR-005**: System MUST always show same-brand alternatives when available, regardless of store
- **FR-006**: System MUST provide confidence scores (0.0-1.0 scale) for each suggestion based on matching criteria, where confidence = normalized(totalScore / maxPossibleScore)
