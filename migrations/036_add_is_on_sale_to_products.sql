...
-- Add is_on_sale column to products table to match Go model
ALTER TABLE products ADD COLUMN IF NOT EXISTS is_on_sale BOOLEAN DEFAULT FALSE;

-- Update is_on_sale based on existing discount data
UPDATE products
SET is_on_sale = TRUE
...
-- Create index for is_on_sale queries
CREATE INDEX IF NOT EXISTS idx_products_on_sale
ON products (is_on_sale, valid_from, valid_to)
WHERE is_on_sale = TRUE AND is_available = TRUE;

-- Recreate hybrid_search_products function to recognize new column
-- This is necessary for PostgreSQL to compile the function with the new column
...
-- Drop index
DROP INDEX IF EXISTS idx_products_on_sale;
